...
public class DorisdbTableRowTransformer implements DorisdbIRowTransformer<RowData> {
    ...
    public DorisdbTableRowTransformer(TypeInformation<RowData> rowDataTypeInfo) {
    ...
    public Object[] transform(RowData record, boolean supportUpsertDelete) {
        ...
        if (supportUpsertDelete) {
            // set `__op` column
            values[idx] = DorisdbSinkOP.parse(record.getRowKind()).ordinal();
        }
        return values;
    }
