...
public class DorisdbSinkOptions implements Serializable {
    ...
    public static final ConfigOption<String> SINK_SEMANTIC = ConfigOptions.key("sink.semantic")
            .stringType().defaultValue(DorisdbSinkSemantic.AT_LEAST_ONCE.getName()).withDescription("Fault tolerance guarantee. `at-least-once` or `exactly-once`");
    ...
    // Sink semantic
    private static final Set<String> SINK_SEMANTIC_ENUMS = Arrays.stream(DorisdbSinkSemantic.values()).map(s -> s.getName()).collect(Collectors.toSet());
    ...
    private DorisdbSinkSemantic sinkSemantic;
    private boolean supportUpsertDelete;

    public DorisdbSinkOptions(ReadableConfig options, Map<String, String> optionsMap) {
    ...
    public DorisdbSinkSemantic getSemantic() {
    ...
    private void validateSinkSemantic() {
        ...
        this.sinkSemantic = DorisdbSinkSemantic.fromName(tableOptions.get(SINK_SEMANTIC));
    }
    ...
    /**
     * Builder for {@link DorisdbSinkOptions}.
     */
    public static final class Builder {
        ...
        public DorisdbSinkOptions build() {
            return new DorisdbSinkOptions(conf, conf.toMap());
        }
    }
}
