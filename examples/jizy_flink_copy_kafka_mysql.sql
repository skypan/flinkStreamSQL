...
CREATE TABLE jizy_flink_kafka (
                                  after_createtime timestamp,
                                  after_id varchar,
                                  after_updatetime timestamp,
                                  after_user varchar
) WITH (
      type = 'kafka',
...
-- 实际执行的sql
insert into jizy_flink_mysql(createtime, updatetime, id,`user`) select after_createtime as createtime, after_updatetime as updatetime, after_id as id, after_user as `user` from jizy_flink_kafka